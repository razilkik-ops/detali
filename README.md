...
На всех основных страницах доступна форма заявки, при этом кнопки прямого звонка сохранены. К заявке можно приложить один чертёж или фотографию в JPG, PNG, WebP либо PDF размером до 8 МБ. Браузер показывает локальное превью изображения и отправляет форму в `hosting/api/submit.php`. PHP-обработчик проверяет фактический MIME-тип вложения, валидирует поля, ограничивает частоту запросов и передаёт заявку с файлом через Telegram Bot API.
